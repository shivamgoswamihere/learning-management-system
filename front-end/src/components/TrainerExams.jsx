              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {exams.map((exam) => (
                  <div key={exam._id} className="bg-white shadow-lg rounded-lg p-5 border border-gray-200">
                    ...
                    <p className="text-gray-700 mb-2"><strong>Date:</strong> {new Date(exam.date).toLocaleDateString()}</p>
                    <p className="text-gray-700 mb-2"><strong>Duration:</strong> {exam.duration} minutes</p>
                    <p className="text-gray-700 mb-2"><strong>Total Marks:</strong> {exam.totalMarks}</p>
                  </div>
                ))}
              </div>
            </div>
